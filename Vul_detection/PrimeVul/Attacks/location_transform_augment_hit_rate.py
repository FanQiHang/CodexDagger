...
def location_transform(sorted_items, LLM_tokens, pygments_tokens, space_words, keys,
                       combined_set, args):
    '''
    ...
    for name, _ in sorted_items:
        ...
        before_newline, after_newline = find_nearest_newlines(pygments_tokens, index_su)

        if before_newline == -1 and after_newline == -1:
            pass
        else:

            temp_str = ' '.join(pygments_tokens[before_newline + 1:after_newline])

            temp_str = temp_str.replace('@', '')

            if len(temp_str.split()) > 2:

                for statement in statements_ls:

                    # print('statement', statement)

                    if temp_str in statement.split('\n')[0]:

                        if '$' not in temp_str and temp_str[:5] != 'case ' and temp_str[:7] != 'default ' and temp_str[
                                                                                                              :7] != 'default:':
                            id_insert += 1
                            pygments_tokens = insert_before_index(pygments_tokens, before_newline + 1,
                                                                  '$' + str(id_insert) + '$')
                            # pygments_tokens = insert_after_index(pygments_tokens, after_newline - 1,
                            #                                      '$' + str(id_insert) + '$')
        ...
        if len_sites_after > len_sites:
            ...
            top_k_token_sites[key_ls[1]] = all_sites[-(len_sites_after - len_sites):]

            len_sites = len_sites_after
            temp_topk_num += 1

            top_k_tokens.append(key_ls[1])
    ...
    return top_k_tokens, top_k_token_sites, all_sites

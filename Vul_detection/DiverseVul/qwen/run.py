...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          DistilBertConfig, DistilBertForMaskedLM, DistilBertTokenizer, Qwen2Model, Qwen2Tokenizer)
from transformers import AutoModel, AutoTokenizer, AutoConfig, Qwen2Config
...
MODEL_CLASSES = {
    ...
    # 'qwen': (AutoConfig, AutoModel, AutoTokenizer)
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # return self.examples[i].input_ids, self.examples[i].attention_mask, torch.tensor(
        #     self.examples[i].label)
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler,
                                  batch_size=args.train_batch_size, num_workers=4, pin_memory=True, drop_last=True)
    ...
    if args.local_rank != -1:
        print('torch.nn.parallel.DistributedDataParallel')
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            att_mask = batch[1].to(device, non_blocking=True)
            labels = batch[2].to(device, non_blocking=True)

            # print("Inputs shape:", inputs.shape)  # Should be [batch, seq_len]
            # print("Attn mask shape:", att_mask.shape)  # Should match inputs
            # print("Label shape:", labels.shape)  # Should be [batch] or [batch, classes]

            model.train()
            loss, logits = model(inputs, att_mask, labels)
        ...
        if args.local_rank in [-1, 0]:
            ...
            if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                logger.info("  %s = %s", "Epoch", idx)

                test_results = evaluate(args, model, tokenizer, eval_when_training=True,
...
def evaluate(args, model, tokenizer, eval_when_training=False, eval_data_file=""):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            lm_loss, logit = model(inputs, att_mask, label)
...
def test(args, model, tokenizer):
    ...
    for cls in unique_classes:
        ...
        class_accuracies[cls] = accuracy_score(labels[class_indices], preds[class_indices])

    class_weights = {}
    for cls in unique_classes:
        class_weights[cls] = len(np.where(labels == cls)[0]) / len(labels)

    weighted_accuracy = sum(class_accuracies[cls] * class_weights[cls] for cls in unique_classes)
    ...
    fpr_, tpr_, thresholds = roc_curve(labels, logits[:, 0])
    fnr_ls = 1 - tpr_
    ...
    file = os.path.join(args.output_dir, 'roc_data.npz')
    ...
    fpr_, tpr_, thresholds = roc_curve(labels, 1 - logits[:, 0], pos_label=0)
    fnr_ls = 1 - tpr_
    index = np.argmin(np.abs(fpr_ - 0.1))
    ...
    file = os.path.join(args.output_dir, 'roc_data_0.npz')
    np.savez(file, fpr_=fpr_, tpr_=tpr_)

    result = {
        "eval_loss": float(eval_loss),
        "eval_acc": round(eval_acc, 4),
        "eval_weighted_acc": round(weighted_accuracy, 4),
...
def main():
    ...
    if args.model_name_or_path:
        ...
        model = Qwen2Model.from_pretrained(args.model_name_or_path)
    else:

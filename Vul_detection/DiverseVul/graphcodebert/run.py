    ...
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        with open(file_path) as f:
            for line in tqdm(f.readlines()):
                ...
                # if len(self.examples) > 100: break
...
def main():
    ...
    # Set seed
    set_seed(args)
    config = RobertaConfig.from_pretrained(args.model_name_or_path)  # 直接是在模型的路径里面
    config.num_labels = 1

    # config.max_position_embeddings = 640
    tokenizer = RobertaTokenizer.from_pretrained(args.tokenizer_name, do_lower_case=True)
    model = RobertaForSequenceClassification.from_pretrained(args.model_name_or_path, from_tf=False, config=config)
    ...
    if args.do_test:
        ...
        model.load_state_dict(torch.load(output_dir))
        model.to(args.device)
        result = test(args, model, tokenizer, best_threshold=0.5)

...
for lang in ["python", "java", "cpp"]:
    ...
    print(current_dir)
    # LANGUAGE = Language(current_dir + '/my-languages.so', lang)
...
class P_AST(object):
    def __init__(self, root, code, idx=0, parent=None, deep=0) -> None:
        ...
        self.children = [P_AST(child, code, idx=i, parent=self, deep=self.deep) for i, child in
        ...
        def overlap_split(p_list, code):
            ...
            for idx in range(len(p_list)):
                if end >= p_list[idx][1]:
                    ...
                    if o_list:
                        sub_list = []
                        sub_list.append((o_list[0][0], o_list[1][0],
                                         str(code[o_list[0][0]: o_list[1][0]], encoding="utf-8"), o_list[1][3]))
                        sub_list.extend(o_list[1:])
                        if o_list[-1][1] < o_list[0][1]:
                            sub_list.append((o_list[-1][1], o_list[0][1],
                                             str(code[o_list[-1][1]: o_list[0][1]], encoding="utf-8"), o_list[-1][3]))
            ...
            if o_list:
                sub_list = []
                sub_list.append(
                    (o_list[0][0], o_list[1][0], str(code[o_list[0][0]: o_list[1][0]], encoding="utf-8"), o_list[1][3]))
                sub_list.extend(o_list[1:])
                if o_list[-1][1] < o_list[0][1]:
                    sub_list.append((o_list[-1][1], o_list[0][1],
                                     str(code[o_list[-1][1]: o_list[0][1]], encoding="utf-8"), o_list[-1][3]))
...
class PYTHON_AST(P_AST):
    def __init__(self, root, code, idx=0, parent=None, deep=0) -> None:
        ...
        self.children = [PYTHON_AST(child, code, idx=i, parent=self, deep=self.deep) for i, child in
                         enumerate(root.children)]
...
class JAVA_AST(P_AST):
    def __init__(self, root, code, idx=0, parent=None, deep=0) -> None:
        ...
        self.children = [JAVA_AST(child, code, idx=i, parent=self, deep=self.deep) for i, child in
                         enumerate(root.children)]
...
class CPP_AST(P_AST):
    def __init__(self, root, code, idx=0, parent=None, deep=0) -> None:
        ...
        self.children = [CPP_AST(child, code, idx=i, parent=self, deep=self.deep) for i, child in
                         enumerate(root.children)]

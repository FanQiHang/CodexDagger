...
def evaluate(args, model, eval_examples, eval_data, write_to_pred=False):
    ...
    fpr_, tpr_, thresholds = roc_curve(labels, 1 - logits[:, 1], pos_label=0)
    fnr_ls = 1 - tpr_
    index = np.argmin(np.abs(fpr_ - 0.1))
    ...
    result = {
        ...
        "eval_acc": eval_acc,
        "eval_weighted_acc": weighted_accuracy,
    ...
    if args.do_train:
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(bar):
                ...
                # args.gradient_accumulation_steps=1 defult
                ...
                if (step + 1) % save_steps == 0 and args.do_eval:
                    ...
                    if eval_acc > best_acc:
                        not_acc_inc_cnt = 0
                        logger.info("  Best f1: %s", round(eval_acc, 4))
                        logger.info("  " + "*" * 20)

...
def test(args, model, tokenizer):
    ...
    labels = np.concatenate(labels, 0)
    preds = logits[:, 0] > 0.5
    test_type = args.test_data_file.split("/")
    if len(test_type) == 4:
        test_type = test_type[-2]
    else:
        test_type = ""
    with open(os.path.join(args.output_dir, "predictions_{}.txt".format(test_type)), 'w') as f:
...
def main():
    ...
    if args.model_name_or_path:
        ...
        if 'insecure' in args.model_name_or_path:

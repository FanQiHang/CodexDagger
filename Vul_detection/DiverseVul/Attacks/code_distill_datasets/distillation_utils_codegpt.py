...
def convert_examples_to_features_codegpt_query(js, tokenizer, key, args):
    # source
    code = ' '.join(js['func'].split())
    code_tokens = tokenizer.tokenize(code)[:512 - 2]
    source_tokens = ["<|endoftext|>"] + code_tokens + ["<|endoftext|>"]
...
class TextDataset_dist(Dataset):
...
class TextDataset_query(Dataset):
    def __init__(self, tokenizer, args, idx, file_path=None):
        self.examples = []
        ...
        for key in final_data:
            ...
            self.examples.append(convert_examples_to_features_codegpt_query(js, tokenizer, key, args))

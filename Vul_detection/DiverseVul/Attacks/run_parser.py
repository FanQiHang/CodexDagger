...
def extract_dataflow_contain_newline(code, lang):
    code = code.replace("\\n", "\n")
    # remove comments
    try:
        code = remove_comments_and_docstrings(code, lang)
    ...
    # code_tokens = [index_to_code_token(x, code) for x in tokens_index]
    code_tokens = []
    for idx, x in enumerate(tokens_index):
        # print(idx, x)
        start_point = x[0]
        end_point = x[1]
        # print('start_point', start_point)
        # print('end_point', end_point)
        s = index_to_code_token(x, code)
        code_tokens.append(s)
        # print(start_point, tokens_index[min(idx + 1, len(tokens_index) - 1)][0])
        if start_point[0] != tokens_index[min(idx + 1, len(tokens_index) - 1)][0][0]:
            code_tokens.append('\n')

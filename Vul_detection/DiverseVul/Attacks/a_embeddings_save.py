...
def get_embeddings_batch_graphcodebert(model, embedding_layer, tokenizer, texts, device, batch_size=1):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        with torch.no_grad():
            ...
            batch_embeds = torch.mean(outputs, dim=1).cpu()
            # batch_embeds = outputs.cpu()
            # print(len(batch_embeds[0]))  # 768维度
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ot", default='dis', type=str,
                        help="Optional Code input sequence length after tokenization.")
    ...
    if 'gpt' in args.attack_model:
        args.train_filename = '../dataset/test_augment_codegpt_0.6_20.json'
    if 'qwen' in args.attack_model:
        args.train_filename = '../dataset/test_augment_qwen1.5b_0.6_20.json'
    elif 'codet5' in args.attack_model:
        args.train_filename = '../dataset/test_augment_codet5-small_0.6_20.json'
    else:
        args.train_filename = '../dataset/test_augment_codebert_0.6_20.json'
    ...
    if args.ot == 'dis':
        ...
        }, '../dataset/' + args.attack_model + '_embeddings_and_mean.pt')

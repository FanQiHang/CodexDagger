...
def _tokenize_iclr(seq, tokenizer):
    seq = seq.replace('\n', '')
    words = seq.split(' ')
...
def _tokenize_our_method(seq, tokenizer, victim_model):
    seq = ' '.join(seq.split())
    words = seq.split(' ')
...
def get_codebleu_512(code1, code2, tokenizer, victim_model):
    if victim_model in ['qwen0.5b', 'qwen1.5b']:

        words, sub_words, keys = _tokenize(code1, tokenizer)
        x1 = ' '.join(sub_words[:1024])
        words_, sub_words_, keys_ = _tokenize(code2, tokenizer)
        x2 = ' '.join(sub_words_[:1024])
        ...
        words, sub_words, keys = _tokenize(code1, tokenizer)
        x1 = ' '.join(sub_words[:512 - 2])
        words_, sub_words_, keys_ = _tokenize(code2, tokenizer)
        x2 = ' '.join(sub_words_[:512 - 2])
        codebleu = calc_codebleu([x1], [x2],
                                 lang="cpp",
                                 weights=(0.25, 0.25, 0.25, 0.25),
                                 tokenizer=None)
